...
function findClauseText(
  ctx: RuleContext,
  patterns: RegExp[]
): { clause: Clause; text: string } | null {
...
export const RENTAL_RULES: RentalRule[] = [
  ...
  },
  {
    ...
    test: ctx =>
      hasProtection(ctx, 'DEPOSIT_REFUND_TIMELINE') === 'absent' ||
      hasProtection(ctx, 'DEPOSIT_REFUND_TIMELINE') === 'unclear',
    severity: () => 'HIGH',
  ...
  },
  {
    ...
    test: ctx => {
      ...
      const found = findClauseText(ctx, [/at any time/i, /without prior notice/i, /no notice/i]);
      return found !== null;
    },
    severity: () => 'HIGH',
  ...
  },
  {
    ...
    severity: ctx => {
      const hasFlatmates = ctx.interview.extras.some(
        e => e.toLowerCase().includes('flatmate') || e.toLowerCase().includes('sharing')
      );
  ...
  },
  {
    ...
    test: ctx => {
      ...
      const isMetro = [
        ...
        'pune',
        'ahmedabad',
      ].some(c => ctx.interview.city?.toLowerCase().includes(c));
